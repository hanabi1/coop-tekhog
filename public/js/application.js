  ...
  // Loop to add stuff from JSON-data (movies variable) to the DOM.
	...
	for (var i = 0;i < movies.length;i++) {
		...
		$('#' + movies[i].link).click(function(){
			...
			$(this).parent().html('<iframe id="' + videoID +'"class="video-player" width="100%" src="//www.youtube.com/embed/'+ videoID +'?modestbranding=1;autoplay=1" frameborder="0" allowfullscreen></iframe>');
...
function resetVideoPlayersToThumbnails(){
	...
	if(videoID){
		...
		$('#' + videoID).click(function(){
			...
			//(this). returns the <img> that was clicked. We cant put the video into the image....
			//So we replace the content in the parent of <img> ie the <li>!
			$(this).parent().html('<iframe id="' + videoID +'"class="video-player" width="100%" src="//www.youtube.com/embed/'+ videoID +'?modestbranding=1&autoplay=1&enablejsapi=1&playerapiid=ytplayer" frameborder="0" allowfullscreen></iframe>');
		});
	}
}
